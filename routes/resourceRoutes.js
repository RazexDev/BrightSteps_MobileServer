...
router.get('/', async (req, res) => {
  try {
    ...
    let query = {};
    if (studentName) {
      query = { $or: [{ studentName: { $exists: false } }, { studentName: null }, { studentName: "" }, { studentName }] };
    }
...
router.put('/:id/access', async (req, res) => {
  try {
    ...
    // Only update and add progress if student hasn't accessed it yet
    if (!resource.accessedBy.includes(studentName)) {
      resource.accessedBy.push(studentName);
      await resource.save();

      // Create a Progress document to count towards the 5-activity Level Up!
      const newProgress = new Progress({
        studentName,
        date: new Date(),
        activity: resource.title,
        mood: 'Happy',
        notes: `Completed Resource: ${resource.title}`,
